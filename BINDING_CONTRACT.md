# expressmax N-API Binding Contract
...
The error handling strategy in expressmax is designed to **prevent any unhandled exception from crashing the Node process** while providing clear, actionable error information to JavaScript.
...
When a JS handler throws, N-API catches the exception and expressmax converts it to a 500 response:
...
| `EXPR_` | expressmax application | `EXPR_ETOOBIG`, `EXPR_EPARSE` |
