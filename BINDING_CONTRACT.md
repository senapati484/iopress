...
The error handling strategy in norvex is designed to **prevent any unhandled exception from crashing the Node process** while providing clear, actionable error information to JavaScript.
...
When a JS handler throws, N-API catches the exception and norvex converts it to a 500 response:
...
| `EXPR_` | norvex application | `EXPR_ETOOBIG`, `EXPR_EPARSE` |
