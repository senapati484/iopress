...
**Objective:** Create the full directory tree, `package.json`, `.gitignore`, and `binding.gyp` for a cross-platform Node.js native addon called `expressmax`.  
...
**Expected outcome:** A TypeScript project can `import ExpressPro from 'expressmax'` with full autocomplete.
...
- [ ] Install: `npm install expressmax` + platform prerequisites (liburing on Linux)
...
**Checklist:**
- [ ] Step 1: Replace `require('express')` with `require('expressmax')`
...
**Expected outcome:** `npm install expressmax` works on Linux/macOS/Windows without compiler installed.
