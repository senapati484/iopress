...
**Objective:** Create the full directory tree, `package.json`, `.gitignore`, and `binding.gyp` for a cross-platform Node.js native addon called `maxpress`.  
...
**Expected outcome:** A TypeScript project can `import ExpressPro from 'maxpress'` with full autocomplete.
...
- [ ] Install: `npm install maxpress` + platform prerequisites (liburing on Linux)
...
**Checklist:**
- [ ] Step 1: Replace `require('express')` with `require('maxpress')`
...
**Expected outcome:** `npm install maxpress` works on Linux/macOS/Windows without compiler installed.
