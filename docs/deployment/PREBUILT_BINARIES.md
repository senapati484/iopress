...
maxpress provides prebuilt binaries for major platforms, eliminating the need for users to install build tools (Python, C++ compiler, etc.).
...
```bash
npm install maxpress --build-from-source=false
```

### Force Compile from Source

```bash
npm install maxpress --build-from-source
```
...
**Solution:**
1. Check if binary exists: `ls node_modules/maxpress/build/Release/`
2. If missing, reinstall: `npm install maxpress --build-from-source`
...
# Test
node -e "const app = require('maxpress')(); console.log('✓ Installed successfully');"
```
...
ls -la node_modules/maxpress/build/stage/

# Verify checksum (if available)
cat node_modules/maxpress/build/stage/*.tar.gz.sha256
```
...
- For Alpine, use: `npm install maxpress --build-from-source`
...
1. Check [GitHub Releases](https://github.com/senapati484/maxpress/releases) for your platform
2. Run `npm run test:prebuilds` for diagnostics
3. Open an issue with:
   - Platform: `uname -a`
   - Node version: `node --version`
   - npm version: `npm --version`
   - Error log: `npm install maxpress --verbose`
