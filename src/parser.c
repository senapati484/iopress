...
int http_parse_request(const uint8_t* buffer, size_t len,
   ...
   *
   * We lowercase the name in-place so JS consumers can do case-insensitive
   * lookups with a single string compare instead of scanning. The buffer
   * is mutable from the server's perspective (it's the read buffer), so
   * this is safe and avoids a copy.
   * =========================================================================
   */
  {
